import 'package:flutter/material.dart';

class AppColors {
  static Color primaryColor = const Color(0xFF3E54FD);
  static Color successColor = const Color(0xFF2E7D32);
  static Color errorColor = const Color(0xFFD32F2F);
  static Color appBarColor = const Color(0xFF17181A);
  static Color textAppBarColor = const Color(0xFFFFFFFF);
  static Color splashScreenBackgroundColor = const Color(0xFFD8F2F3);
  static Color textColor = const Color(0xFF1D1B20);
  static Color textPriceColor = const Color(0xFF04283B);
  static Color white = const Color(0xFFFFFFFF);
  static Color lightWhite = const Color(0xFFF4F4FA);
  static Color yellow = const Color(0xFFF6E266);
  static Color grey = const Color(0xFF46494E);
  static Color red = const Color(0xFFCF002B);
  static Color orange = const Color(0xFFFF4E1F);
  static Color lightOrange = const Color(0xFFE0E0E0);
  static Color title = const Color(0xFF333337);
  static Color black = const Color(0xFF000000);
}
